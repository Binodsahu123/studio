...
export default function HtmlConverterPage() {
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    ...
    try {
      ...
      setHtmlResult(result.htmlContent);
    } catch (error) {
  ...
  const handleCopy = (text: string, type: 'HTML' | 'Text') => {
    navigator.clipboard.writeText(text).then(() => {
      if (type === 'HTML') {
        ...
        toast({ title: "Copied plain text to clipboard!" });
              ...
              <CardHeader>
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <CardDescription>Convert plain text into well-structured, SEO-friendly HTML automatically.</CardDescription>
                  </div>
                </div>
              </CardHeader>
              <CardContent>
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          ...
                          <FormControl>
                            <Textarea placeholder="Paste your article, notes, or any plain text here..." {...field} rows={12} />
                          </FormControl>
                          <FormMessage />
                        </FormItem>
                      )}
                     ...
                     {htmlResult && (
                        <div className="flex gap-2">
                           <Button variant="outline" size="sm" onClick={handleCopyText} disabled={isTextCopied}>
                            ...
                            <Button variant="outline" size="sm" onClick={() => handleCopy(htmlResult, 'HTML')} disabled={isHtmlCopied}>
